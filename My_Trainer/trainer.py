...
# )
# 可选导入助手：缺失就用兜底
def _optional_integration(name, fallback):
    try:
        import transformers.integrations as _i
        return getattr(_i, name)
    except Exception:
        return fallback

# 逐个绑定，缺失时给出安全兜底
default_hp_search_backend = _optional_integration(
    "default_hp_search_backend", lambda *a, **k: None
)

get_reporting_integration_callbacks = _optional_integration(
    "get_reporting_integration_callbacks", lambda *a, **k: []
)

hp_params = _optional_integration(
    "hp_params", lambda *a, **k: {}
)

is_fairscale_available = _optional_integration(
    "is_fairscale_available", lambda: False
)
is_optuna_available = _optional_integration(
    "is_optuna_available", lambda: False
)
is_ray_tune_available = _optional_integration(
    "is_ray_tune_available", lambda: False
)
is_sigopt_available = _optional_integration(
    "is_sigopt_available", lambda: False
)
is_wandb_available = _optional_integration(
    "is_wandb_available", lambda: False
)

def _not_impl(*a, **k):
    raise RuntimeError("This HP search backend is not available in the current transformers version.")

run_hp_search_optuna = _optional_integration("run_hp_search_optuna", _not_impl)
run_hp_search_ray    = _optional_integration("run_hp_search_ray",    _not_impl)
run_hp_search_sigopt = _optional_integration("run_hp_search_sigopt", _not_impl)
run_hp_search_wandb  = _optional_integration("run_hp_search_wandb",  _not_impl)
